...
def run(job_data, account_data, keychain_name):
    ...
    report_progress(1, "Initializing signing job")
    ...
    report_progress(5, "Setting up environment")

    with tempfile.TemporaryDirectory() as temp_dir_str:
        temp_dir = Path(temp_dir_str)
        print("Extracting app...")
        report_progress(8, "Extracting IPA package")
        extract_zip(Path("unsigned.ipa"), temp_dir)
        report_progress(12, "IPA extracted successfully")

        tweaks_dir = Path("tweaks")
        if tweaks_dir.exists():
            print("Found tweaks, injecting...")
            report_progress(14, "Injecting tweaks into app")
            inject_tweaks(temp_dir, tweaks_dir)
            report_progress(16, "Tweaks injected successfully")

        print("Starting signing process...")
        report_progress(18, "Preparing for signing")
        ...
        report_progress(75, "All components signed successfully")
        print("Packaging signed app...")
        signed_ipa = Path("signed.ipa")
        archive_zip(temp_dir, signed_ipa)
        report_progress(78, "Signed IPA package created")

    report_progress(80, "Preparing to upload signed IPA")
    ...
    report_progress(95, "Upload completed, finalizing job")
